...
My advisor, professor Alexandre Linhares, has proposed another read operation: the weighted reading. In it, the sum of the counter's value is weighted based on the distance between the hard-location's address and the reading address. The logic behind it is to change the importance of each hard-location inside the circle. We have tested how SDM behaves when the weight is calculated using the information given by each bitstring, i.e., $w(d) = -\log_2 \left( 2^{-N} \binom{N}{d} \right) = N - \log_2 \binom{N}{d}$, as in information theory \citep{cover2012elements}.
...
For further information about critical distance behavior, see \citet{Brogliato2011} and \citet{brogliato2014sparse}.
