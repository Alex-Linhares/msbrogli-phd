...
\begin{enumerate}
    ...
    \item Learning Intelligent Distribution Agent (LIDA) developed by Stan Franklin's lab at the University of Memphis includes implementation of SDM in Java: http://ccrg.cs.memphis.edu/framework.html;

\end{enumerate}

Let us analyse these.  The Connection Machine is obsolete.  The NASA implementation is hardware-based, hence not open-source.  APL, while influential, is not a mainstream language in science.
...
0
    ...
    \item There are no publishable / published scientific applications or experiments available to be reproduced.
...
The main contribution of this work is a reference implementation which yields (i) orders of magnitude gains in performance, (ii) has several backends\footnote{CPUs, GPUs, FPGAs} and operations, (iii) is fully validated against the mathematical model, (iv) is cross-platform\footnote{Unix, Linux, MacOs, Windows, Amazon Web Services, etc.}, and (v) is easily extensible to test new research ideas --- and to let others replicate the results.
...
% \subsection{Deep learning, multiple SDMs, and the animal behavior of Dr. Linhares}
...
\section{Illuminating, paving, clearing}
