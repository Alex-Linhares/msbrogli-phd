...
\begin{tabular}{cp{\textwidth}}
  ...
  $N'$ & Number of hard-locations samples from $\{0, 1\}^n$. It typical value is 1,000,000. \\
  $H$ & The same as $N'$ \\
  $r$ & Access radius, i.e., when $n=1,000$ and $N'=1,000,000$, it typical value is $451$. This value is calculated to activate, on average, one thousand of $N'$. \\
  $\eta$ & A bitstring \\
...
Consider the following. Given any two $n$-sized bitstrings, the probability of a Hamming distance $d$ is given by,

%\begin{align*}

$p(H=d)=\frac{ \binom{n}{d} }{ 2^n },$ \\

$p(H\leq d)={\frac{1}{2^n}}{\displaystyle\sum_{i=0}^{d}{\binom{n}{i}}  }$, \\

%\end{align}

and, consequently,

$p(H\geq n-d)={\frac{1}{2^n}}{\displaystyle\sum_{i=n-d}^{n}{\binom{n}{i}}  }$, \\
$p(d+1 \leq H \leq n-d-1)=2^n - {\frac{1}{2^{n-1}}}{\displaystyle\sum_{i=0}^{d}{\binom{n}{i}}  .}$ \\

accordingly, we have tested how SDM behaves when write weights are calculated using the amount of information given by each bitstring, i.e., $w(d) = -\log_2 \left( 2^{-n} \binom{n}{d} \right) = n - \log_2 \binom{n}{d}$, as in information theory \citep{cover2012elements}.
...
\subsection{Symmetrical Hard Locations}
