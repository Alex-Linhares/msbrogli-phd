...
% Describe the basics of SHA-256 algorithm.
...
%As one may fail many times until they find a solution, the probability of finding a solution in the $K$ trial follows a geometric distribution. Let $X$ be the number of trials until a success, then $\mathbf{P}(X = k) = (1-p)^{k-1} p$, where $p = \mathbf{P}(a < A)$, i.e., there would be $k-1$ failures until a success in the $k-th$ trial. The average number of trials is $\mathbf{E}(X) = 1/p$ and the variance is $\mathbf{V}(X) = (1-p)/p^2$.
...
% A partir de qual valor de H é razoável dizer que T segue uma distribuição exponencial?
\begin{theorem}
...
That way, it is not exacly correct to say that when one doubles their hash rate, their probability will double as well. It is only true for small miners.

%Then, $\mathbf{P}(t_0 < T < t_1) = \mathbf{P}(t_0 < X/H < t_1) = \mathbf{P}(t_0 H < X < t_1 H) = [1 - (1-p)^{t_1 H}] - [1 - (1-p)^{t_0 H}] = (1-p)^{t_0 H} - (1-p)^{t_1 H}$.
...
It is easy to notice that $lim_{a \rightarrow 0} \mathbf{E}[T] = 600$, and $lim_{a \rightarrow \infty} \mathbf{E}[T] = 0$.
...
\end{cor}
\begin{proof}
	This probability $\mathbf{P}(S = s) = a_{00}$.
\end{proof}

Assuming that the attacker starts the attack just after publishing the transaction, the probability of the attacker has already found more than $k$ blocks while it waits the network to find $k$ blocks is $\mathbf{P}(S \geq k) = \sum_{s=k}^{\infty} \binom{k+s-1}{s} (1-p)^s p^k$.
