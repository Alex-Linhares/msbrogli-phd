...
The fact that the time between blocks follows an exponential distribution with $\lambda = 1/\tau = pH$ may be used to estimate the total network's hash rate (or a miner's hash rate). For further information, see \cite{ozisikestimation}.
...
Let's analyze what would happen if the network's hash rate is growing linearly with angular coefficient $a^2$, i.e., $u(t) = 1 + a^2t$. Thus, $\mathbf{P}(T \leq t) = 1 - e^{-\frac{t + a^2t^2/2}{\tau}}$.

It is well known that $\mathbf{E}(T) = \int_{0}^{\infty} 1 - \mathbf{P}(T \leq t) dt$. Thus, replacing $y = \frac{a^2 t + 1}{a \sqrt{2 \tau}}$, and using the fact that $\int_0^\infty e^{-x^2} dx = \sqrt{pi}/2 \erf(x)$, we have:

\begin{align}
\mathbf{E}(T)|_{t_1}^{t_2} &= \int_{t_1}^{t_2} \exp \left( - \frac{t + a^2 t^2/2}{\tau} \right) dt \nonumber \\
	&= \frac{\sqrt{2 \tau}}{a} \exp \left( \frac{1}{2a^2 \tau} \right) \int_{y_1}^{y_2}  \exp(-y^2) dy \nonumber \\
	&= \frac{\sqrt{2 \tau}}{a} \exp \left( \frac{1}{2a^2 \tau} \right) \frac{\sqrt{\pi}}{2} [\erf(y_1) - \erf(y_2)] \nonumber \\
	&= \frac{\sqrt{2 \pi \tau}}{2a} \exp \left( \frac{1}{2a^2 \tau} \right) \left[\erf(y_2) - \erf(y_1) \right]
\end{align}

Where $y_1 = \frac{a^2 t_1 + 1}{a \sqrt{2 \tau}}$ and $y_2 = \frac{a^2 t_2 + 1}{a \sqrt{2 \tau}}$.

Thus, $\mathbf{E}(T) = \mathbf{E}(T)|_0^\infty$. When $t_1 = 0 \Rightarrow y_1 = \frac{1}{2 \sqrt{2 \tau}}$ and $t_2 \rightarrow \infty \Rightarrow y_2 \rightarrow \infty \Rightarrow \erf(y_2) = 1$, then:

$$
\mathbf{E}(T) = \frac{\sqrt{2 \pi \tau}}{2a} \exp \left( \frac{1}{2a^2 \tau} \right) \left[1 - \erf\left(\frac{1}{a \sqrt{2 \tau}}\right)\right]
$$

The function $\mathbf{E}(T)|_{t_1}^{t_2}$ may be used to a piecewise linear analysis of any hash rate change. Let's analyze the hash doubling in one week. Then, $u(1 \text{week}) = u(604,800) = 2$, thus $a^2 = \frac{1}{604,800} \Rightarrow a = \frac{1}{120 \sqrt{42}}$. Let's partition the interval $[0, 1 \text{week}]$ in hourly intervals, i.e., ($t_0$, $t_1$, $t_2$, \dots, $t_168$), where $t_i = i \cdot 1 \text{hour} = i \cdot 3600$. For each interval $[t_i, t_{i+1}]$, we evaluate $\mathbf{E}(T)|_{t_i}^{t_{i+1}}$. The result is presented in Figure .
...
There are many possible ways to attack the Bitcoin (REF). In this section, we are interested in a particular attack: the submarine attack. 

In the submarine attack, the attacker's goal is to spend some Bitcoins and, after a while, get their Bitcoins back. Thus, the attacker sends some funds to the victim, they wait 6 confirmations, and, finally, the victim confirms the funds. But, then, the attacker get their funds back propagating new blocks which accept a double spending transaction. In order to work, the attacker must propagate more blocks than the network has mined in the same period, becoming the longest chain. Hence, we would like to understand what the odds are that the attacker will be successful.

In order to maximize their odds, the attacker must start to mine the new blocks as soon as they send the funds to the victim. In this moment, it starts to mine in the head of the blockchain, just like the rest of the network. So, the attacker and the network are in exactly the same point.
