...
\caption{The threshold size $r$ of hard-locations bring about the autocorrelation of Theorem \ref{T1}. Consider, for instance, the extremes \{0, $n$\}.  Let us start with $n$ (in our case, $n=1000$ dimensions): given the information that $d(x,y)\le n$, the probability of a bitmatch in dimension $i$ is $p=1/2$; as $\forall x,y \in \{0, 1\}^N, d(x,y)\le n$. At the other extreme, consider that we have the information that $d(x,y)\le 0$: in this case $x=y$ and the probability of a bitmatch, in any dimension, is $p=1$. The autocorrelation hence drops monotonically until convergence at $p=1/2$ as the distance grows.
\label{fig:bitmatchbias}}
\end{figure}

A natural question is what happens to this pull bias as $n$ grows?  Let $f = P(d(x,y)\le r) = 2^{-n}\sum_{k=0}^{r}{\binom{n}{k}}$.  Then, we have:

\begin{lemma}
Let $r$ be the access radius given that $f$ is the ratio of the hard locations activated. Then, $\lim_{n \rightarrow \infty} r/n = 1/2$.
...
Let $\Phi(x)$ be the cdf of the standard normal distribution. If $z = \frac{r - n/2}{\sqrt{n}/2}$, $z$ is, hence, measuring the number of standard deviations from the average in the Gaussian\footnote{MARCELO, CHEQUE ESTA ULTIMA FRASE PLEASE?}.  Thus, $P(d(x, y) \le r) = \Phi(z)$. As $f = P(d(x, y) \le r)$, then, $f = \Phi(z)$ for our fixed $r$}.
...
As seen on Fig. \ref{fig:bitmatchbias}, the bias disappears as $r$ grows.  Numerically, our results converge to precisely $p=1/2$ only after $d(x,y)\ge 83$ for $n=100$, $d(x,y)\ge 607$ for $n=1000$; $d(x,y)\ge 5332$ for $n=10000$. This can be seen in Lemma \ref{thm:sdm-access-radius}.
...
In order to check this probability, we have run a simulation reading from 1,000 random bitstrings (which have never been written into memory) and calculated the distance from the result of a single read. As the $P(wrong) = 0.22037$, we expected to get an average distance of 220.37 with a standard deviation of 13.10. See Figure \ref{fig:sdm-read-random-bs} for the comparison between the simulated and the theoretical outcomes.
